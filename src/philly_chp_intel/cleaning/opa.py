...
def clean_opa_properties(df: pd.DataFrame) -> pd.DataFrame:
    ...
    address = pick_first_column(raw, ["street_address", "address", "mailing_address"]).map(normalize_address)
    ...
    building_type = pick_first_column(raw, ["market_value_description", "use_code_description", "building_type"])
